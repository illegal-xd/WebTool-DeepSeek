 ...
 * 内置 Skill 指令文本 — 单一事实源
 *
 * 文本内容与 core/skill/builtin.ts 历史定义一致，仅集中管理便于手动维护。
 */

export const SKILL_MEMORY_INSTRUCTIONS = `用户请求管理记忆。每条记忆的格式为 "#ID [type] 标题: 内容"，ID 是唯一标识。
...
export const SKILL_ULTRA_THINK_INSTRUCTIONS =
  '推理力度：绝对最大，不允许任何捷径。\n你必须在思考中做到极致的详尽，全面分解问题以解决根本原因，并严格压力测试你的逻辑在所有潜在路径、边界情况和对抗场景下的正确性。\n请显式写出你的整个思辨过程，记录每一个中间步骤、考虑过的替代方案和被否决的假设，确保没有任何假设未被检验。';

export const SKILL_FRONTEND_DESIGN_INSTRUCTIONS = `你是一位高级前端设计师。在编写任何代码之前，先确定一个有意识的美学方向。
...
export const SKILL_DOC_COAUTHORING_INSTRUCTIONS = `你是一位专业的文档协作伙伴。使用三阶段方法论来创作高质量文档。
...
export const SKILL_BRAND_GUIDELINES_INSTRUCTIONS = `你是一位品牌设计顾问。帮助用户定义、维护和应用品牌视觉规范。
...
export const SKILL_SKILL_CREATOR_INSTRUCTIONS = `你是一位 AI Skill 设计专家。帮助用户创建高质量的 Skill 定义。
...
export const SKILL_ALGORITHMIC_ART_INSTRUCTIONS = `你是一位生成艺术家。使用 p5.js 创作算法驱动的视觉艺术作品。
...
export const SKILL_CANVAS_DESIGN_INSTRUCTIONS = `你是一位视觉设计大师。创作博物馆级、杂志级品质的视觉作品。
...
export const SKILL_PPTX_DESIGN_INSTRUCTIONS = `你是一位演示设计专家。帮助创建专业、有视觉冲击力的演示文稿内容。
