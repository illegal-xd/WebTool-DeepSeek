...
export type McpServerCreateInput = Pick<McpServerConfig, 'displayName' | 'transport'> & Partial<Pick<McpServerConfig, 'enabled' | 'headers' | 'secrets' | 'timeouts' | 'limits' | 'allowlist' | 'execution'>>;
export type McpServerUpdateInput = Partial<Pick<McpServerConfig, 'displayName' | 'enabled' | 'transport' | 'headers' | 'secrets' | 'timeouts' | 'limits' | 'allowlist' | 'execution' | 'status' | 'lastConnectedAt' | 'lastError'>>;
...
export interface McpProtocolTransport {
  request<TParams extends Record<string, unknown> | undefined, TResult>(request: McpJsonRpcRequest<TParams>, options?: { timeoutMs?: number }): Promise<McpJsonRpcResponse<TResult>>;
  notify?(notification: McpJsonRpcNotification, options?: { timeoutMs?: number }): Promise<void>;
}
